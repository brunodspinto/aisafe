...
- **Creator vs assigned pilot** — Two ownership rules are enforced by the controller, both before the aggregate is created: (1) the *assigned* pilot must belong to the route's company (the explicit acceptance criterion AC080.3); and (2) the *authenticated* pilot (the creator) may only create plans for routes of their own company. Rule (2) is a security boundary — the UI already scopes the selectable routes to the creator's company, but the controller enforces it for any caller (it throws `"You can only create flight plans for your own company's routes."`).
