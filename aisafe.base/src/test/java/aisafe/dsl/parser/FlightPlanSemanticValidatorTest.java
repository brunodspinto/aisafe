...
class FlightPlanSemanticValidatorTest {
    ...
    @Test
    void ensureSemanticErrorsProducedByParserKeepSourcePosition() {
        final String dsl = """
        ...
        final var result = new FlightPlanParserFacade().parse(dsl);

        assertFalse(result.isValid());
        assertTrue(result.errors().stream().anyMatch(error ->
                error.message().contains("arrival airport")
                        && error.line() == 17
                        && error.column() == 4));
    }
}
